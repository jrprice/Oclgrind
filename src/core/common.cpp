...
void getConstantData(unsigned char* data, const llvm::Constant* constant)
{
  ...
  case llvm::Type::StructTyID:
  {
    ...
    for (unsigned i = 0; i < num; i++)
    {
      unsigned offset = getStructMemberOffset((const llvm::StructType*)type, i);
...
llvm::Instruction* getConstExprAsInstruction(const llvm::ConstantExpr* expr)
{
  ...
  default:
    ...
    llvm::BinaryOperator* binaryOp = llvm::BinaryOperator::Create(
      (llvm::Instruction::BinaryOps)opcode, operands[0], operands[1]);

    // Check for overflowing operator
    if (opcode == llvm::Instruction::Add || opcode == llvm::Instruction::Mul ||
        opcode == llvm::Instruction::Shl || opcode == llvm::Instruction::Sub)
    {
    ...
    if (opcode == llvm::Instruction::AShr ||
        ...
        opcode == llvm::Instruction::SDiv || opcode == llvm::Instruction::UDiv)
    {
...
unsigned getTypeSize(const llvm::Type* type)
{
  ...
  else if (type->isVectorTy())
  {
    ...
    if (num == 3)
      num = 4; // Hack for 3-element vectors
    return num * sz;
  }
