...
struct FillImageCommand : Command
{
  ...
  size_t pixelSize;
  unsigned char* color;
  FillImageCommand(size_t b, const size_t o[3], const size_t r[3], size_t rp,
                   size_t sp, size_t ps, const unsigned char* col)
  {
    type = FILL_IMAGE;
    base = b;
...
struct NativeKernelCommand : Command
{
  ...
  void* args;
  NativeKernelCommand(void(CL_CALLBACK* f)(void*), void* a, size_t sz)
  {
    type = NATIVE_KERNEL;
    func = f;
    if (a)
